...
function getItem(i){
	var displayContent = "";
	if(contentArray[i].length > 15) {
		displayContent = contentArray[i].substring(0, 15) + "...";
	}
	else{
		displayContent = contentArray[i];
	}

	return "<a href='EditNotice.html?doctorNoticeID="+doctorNoticeIDArray[i]+"' class='list-group-item left-list-item notice'>" +
				"<h4 class='list-group-item-heading'>" + displayContent + "</h4>" +
			"</a>";
}